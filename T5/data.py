...
class EventDataset(Dataset):
    def __init__(self, logger, args, data, tokenizer, mode):
        self.data = data
        self.tokenizer = tokenizer
        self.topology = {"entity-entity": True, 
                           "entity-relation": True,
                           "relation-entity": True,
...
#         with open(self.data_path+ '.target_eval') as f:
#             target_texts = [sample.rstrip('\n') for sample in f]
        ...
        self.data_type = mode
        self.metric = "BLEU"
        self.head_ids, self.rel_ids, self.tail_ids = self.tokenizer.encode(' <H>', add_special_tokens=False), \
    ...
    def graph_size(self,idx):
        ...
        kg_list = []
        triple_list = kg.split('<H>')
        triple_list = [triple_list[0]] + ['<H>'+triple for triple in triple_list[1:]]
        triple_list = list(filter(None,triple_list))
        for triple in triple_list:
            head = re.search('<H>(.*)<R>', triple).group(1).strip()
            rel = re.search('<R>(.*)<T>', triple).group(1).strip()
            tail = re.search('<T>(.*)', triple).group(1).strip()
    ...
    def graph_linearize(self, idx, triple, entity_change, head_ids, rel_ids, tail_ids,
                        relation_change, cnt_edge, adj_matrix):
        # string_label: encoder ids
        # string_label_tokens: encoder tokens
        if len(triple[0]) == 0:
        ...
        nodes, edges = [], []
        string_label = copy.deepcopy(head_ids)
        string_label_tokens = ' <H>'
        nodes.extend([-1] * len(string_label))
        edges.extend([-1] * len(string_label))

        # print("entity_change: ", entity_change, "idx: ", idx)
        # print("triple: ", triple)
        try:
            string_label += entity_change[triple[0]][0]
        except:
            print("entity_change: ", entity_change, "idx: ", idx, "triple: ", triple)
            raise
        ...
        if len(triple[1]) != 0 and len(triple[2]) != 0:
            ...
            words_label_tokens = ' <R> {} <T> {}'.format(rel_label_token, triple[2])
            nodes.extend(
    ...
    def get_all_entities_per_sample(self, triple_list):
        # print("in get_all_entities_per_sample")
        text_entity = set()
        text_relation = set()
        for triple in triple_list:
            if len(triple[0]) == 0:
